...
const networks = {
  sepolia: {
    ...
    rpcUrls: [
      'https://rpc.ankr.com/eth_sepolia/86add4e60d19755c26da6dd7566a82a53b3d8e39222787f3efb6f646148f1864',
    ],
...
const FETCH_CONTRACT = (address, abi, signer) => new ethers.Contract(address, abi, signer)
...
export const GET_ALL_REGISTERED_MEDICINES = async () => {
  ...
  const _medicinesArray = await Promise.all(
    medicines.map(async ({ id, IPFS_URL, price, quantity, discount, currentLocation, active }) => {
      const {
        data: {
          verifyingDoctor,
          name,
...
export const GET_ALL_PRESCRIBED_MEDICINES_OF_PATIENT = async (_prescriptionId) => {
  const address = await CHECKI_IF_CONNECTED()
  const contract = await HEALTH_CARE_CONTARCT()
  ...
  if (address) {
    ...
    const _prescriptionArray = Promise.all(
      prescription.map(async ({ id, medicineId, patientId, doctorId, date }) => {
        const patient = await GET_PATIENT_DETAILS(Number(_patientID))
...
export const GET_NOTIFICATION = async (_address) => {
  ...
  const _notificationArray = await Promise.all(
    notifications.map(async ({ id, userAddress, message, timestamp, categoryType }) => {
      return {
